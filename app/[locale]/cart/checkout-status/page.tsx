...
//   // ✅ Order success (no redirect URL)
//   if (saveOrderData?.success === true && saveOrderData?.data?.url == null) {
//     dispatch(getCartProducts() as any);
//     return (
//       <PageWrapper>
...
export default function CheckoutStatusPage() {
  ...
  // Local state for preloading (before Redux data fully loads)
  const [isInitializing, setIsInitializing] = useState(true);

  // ✅ Simulate page initialization (for SSR hydration, data setup, etc.)
  useEffect(() => {
    // Wait a short moment to ensure Redux has loaded from storage (if using persist)
  ...
  // ✅ Redirect if checkout data missing
  useEffect(() => {
    if (!isLoading && !status && !saveOrderData && !isInitializing) {
      router.replace("/home");
    }
  }, [status, isLoading, saveOrderData, router, isInitializing]);

  // ✅ Redirect to external payment if success URL exists
  useEffect(() => {
    const success = saveOrderData?.data?.data?.success;
    const url = saveOrderData?.data?.data?.url;
    if (success && url) {
  ...
  // ✅ PRELOAD PHASE: before Redux data or router settles
  if (isInitializing) {
    return (
      <PageWrapper>
        <div className="flex flex-col items-center justify-center min-h-[400px] animate-pulse">
          <p className="text-gray-600 text-base sm:text-lg">
            {t("loading-checkout-status")}...
          </p>
        </div>
      </PageWrapper>
    );
