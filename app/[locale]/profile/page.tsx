...
export default function ProfilePage() {
  ...
  console.log('Profile page - User keys:', user ? Object.keys(user) : 'No user');
  ...
  if (typeof window !== 'undefined') {
    console.log('Profile page - localStorage accessToken:', !!localStorage.getItem('accessToken'));
    console.log('Profile page - localStorage refreshToken:', !!localStorage.getItem('refreshToken'));
  }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          <div className="mb-8 text-center">
            <h1 className="text-3xl font-bold text-navy-blue-color mb-2">{t('title')}</h1>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-navy-blue-color">{t('title')}</CardTitle>
              <CardDescription>{t('subtitle')}</CardDescription>
            </CardHeader>
            <CardContent>
                          <FormProvider {...form}>
                             <form onSubmit={form.handleSubmit(onSubmit)} className={`space-y-6 ${isRTL ? 'rtl' : 'ltr'}`}>
                ...
                <div className="flex justify-center pt-6">
                  <Button
                    type="submit"
                    className="bg-navy-blue-color hover:bg-navy-blue-color/90 text-white px-8 py-3"
                    disabled={form.formState.isSubmitting}
                  >
                    {form.formState.isSubmitting ? 'جاري الحفظ...' : t('saveChanges')}
                  </Button>
                </div>
              </form>
            </FormProvider>
