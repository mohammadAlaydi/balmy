...
const SearchPopup = ({ products }: { products: any[] }) => {
  return (
    <div className="w-full h-full flex flex-col gap-5 py-3 overflow-y-auto">
      ...
      <div className="w-full h-full flex flex-wrap  gap-2">
        {products?.map((product, index) => (
          <ProductCard key={index} product={product} width="w-fit" />
        ))}
      </div>
    </div>
  );
