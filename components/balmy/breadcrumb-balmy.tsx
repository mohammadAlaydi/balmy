                    ...
                    return (
                        <li key={`${item.label}-${index}`} className="flex items-center gap-2">
                            {isLast ? (
                                ...
                                <Link
                                    href={item.href}
                                    className="rounded text-[var(--color-medium-gray)] transition hover:opacity-80 focus:outline-none focus:ring-2 focus:ring-black/20"
                                >
                                    {item.label}
                                </Link>
                            ) : (
                                <span className="text-[var(--color-medium-gray)]">{item.label}</span>
                            )}

                            {!isLast ? (
                                <span className="text-[var(--color-light-gray-6)]" aria-hidden>
                                    /
                                </span>
                            ) : null}
                        </li>
